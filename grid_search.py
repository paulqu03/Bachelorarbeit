'''
Dieses Programm führt eine Grid Search zwischen tau_I und v_E_I durch. Das Programm kann aber auch für die anderen Grid Searches
durchführen. Dafür müssen aber mehrere Stellen im Code angepasst werden:
    - base_path und sim_folder
    - Wertebereich und Einheit (GRID_VALUES)
    - betroffene Bereiche des Modells, welche entfernt und wieder hinzugefügt werden müssen
    - Achsenbeschriftung der Heatmaps, sowie Einheiten der GRID_VALUES in der Heatmap

Jede Parameterkombination wird 20-mal simuliert.  Für jede Simulation werden dabei ein Spike-Raster-Plot, ein Spike-Activity-Plot 
und ein Stimulus-Plot erstellt. Zudem werden für jede Metrik am Ende eine jewweilige Heatmap erstellt. 
Die Durchführung eines Grid Searches kann dabei bis zu 6h dauern. Die Ergebnisse des Grid Search können auch von den Ergebnissen
der Arbeit abweichen, durch eine andere Modellstruktur oder insgesamter Inputvariation.
...
'generate_fixed_synapse_connections.py' ausgeführt werden. Zudem ssind die Metriken momentan darauf ausgelegt mit einem E1 Bias zu rechnen.s
...
base_path = Path("./ergebnisse/grid_search/v_E_I")
...
for tau_idx, tau_I in enumerate(TAU_I_VALUES):
    ...
    for val_idx, v_val in enumerate(GRID_VALUES):
        # Gesamtes Modell auf jede Parameterkombination anpassen
        net.restore()        
        
        net.remove(I, S_E1_I, S_E2_I, S_I_I, S_I_E1, S_I_E2, S_P_I, PR_I, SM_I)
        I  = NeuronGroup(N_I,  'dv/dt = -v / tau_I : volt (unless refractory)',
                threshold='v >= v_th_I', reset='v = v_reset',
                method='euler', refractory=t_ref, name='I')
        ...
        net.add(I, S_E1_I, S_E2_I, S_I_I, S_I_E1, S_I_E2, S_P_I, PR_I, SM_I)
        net.store()


        true_pos, comp_cnt, predec_cnt = 0, N_RUNS, 0
        delta_abs_list, i_eff_list = [], []
        win_act_list, lose_act_list = [], []
        ...
        for run_idx in range(N_RUNS):
            ...
            sim_folder = base_path / f"tau_I_{int(tau_I/ms)}ms" / f"v_E_I_{round(v_val/mV, 1)}" / f"simulation_{run_idx+1}"
            ...
            ax1.set(xlabel='Zeit (ms)', ylabel='Feuerrate (Hz)',
                    title=f'Spike rates (bin {BIN_SIZE/ms} ms) {round(v_val,2)}')
            ...
            ax2.set(xlabel='Zeit (ms)', ylabel='Neuron Index', title='Spike raster', xlim=(0, runtime/ms))
            ...
            ax3.set(xlabel='Zeit (ms)', ylabel='Rate (Hz)'); ax3.legend(); fig3.tight_layout()
        ...
        sensitivity_map[tau_idx, val_idx] = 100-(comp_cnt / N_RUNS * 100)
        over_sens_map[tau_idx, val_idx]   = predec_cnt / N_RUNS * 100


# Hilfsfunktion zum plotten und speichern der Heatmaps
def save_heatmap(matrix: np.ndarray, title: str, cmap: str, fname: str,
    ...
    im = ax.imshow(matrix, origin='lower', aspect='auto', cmap=cmap,
                   extent=(GRID_VALUES[0]/mV, GRID_VALUES[-1]/mV, #müssen je nach Gridsearch angepasst werden (Einheit)
                           TAU_I_VALUES[0]/ms, TAU_I_VALUES[-1]/ms),
                   vmin=vmin, vmax=vmax)
    ax.set_xlabel('v_E_I (mV)')
    ax.set_ylabel('tau_I (ms)')
    ax.set_title(title)
    cbar = fig.colorbar(im, ax=ax)
    cbar.set_label('%' if percent else 'value')
    fig.tight_layout()
...
save_heatmap(clarity_map,       'Eindeutigkeits Heatmap',          'plasma',   'heatmap_activity_difference.png',          vmin=0, vmax=1400)
save_heatmap(eff_inhib_map,     'Effektive Inhibition Heatmap',    'seismic',  'heatmap_eff_inhib.png',                    vmin=-30, vmax=30)
save_heatmap(sensitivity_map,   'Stabilitäts Heatmap',             'RdYlGn',   'heatmap_decision_making_competition.png',  percent=True)
save_heatmap(over_sens_map,     'Vorentscheidungs Heatmap',        'RdYlGn_r', 'heatmap_pre_stimuli_decision.png',         percent=True)

# Für eventuelle Anpasssungen in Pickle-Datei peichern
with open(heatmap_dir / 'heatmaps_data.pkl', 'wb') as f:
    pickle.dump({
        'accuracy': accuracy_map,
        'clarity': clarity_map,
        'eff_inhibition': eff_inhib_map,
        'winner_clarity': win_clarity_map,
        'loser_clarity': lose_clarity_map,
        'sensitivity': sensitivity_map,
        'over_sensitivity': over_sens_map
    }, f)
